...
void vm_lru(void *upage){
  struct list_elem *e;
  for (e = list_begin (&frame_list); e != list_end (&frame_list); e = list_next (e)){
    struct frame_table_entry *t = list_entry (e, struct frame_table_entry, lelem);
    if(t->upage == upage){
      struct list_elem* a;
      a = list_remove(e);
      list_push_back(&frame_list, &(t->lelem));
      break;
    }
  }
}
...
struct frame_table_entry* pick_frame_to_evict( uint32_t *pagedir )
{
  ...
  #ifdef FIFO
  struct frame_table_entry *e = fifo();
  return e;
  #endif
