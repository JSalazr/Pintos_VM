...
int find_thread(struct mini_thread *mt){
  ...
  for (e = list_begin (&all_list); e != list_end (&all_list); e = list_next (e)){
    ...
    if(t->tid == mt->tid && t->status != THREAD_DYING){
      ...
      mt->cant_page_faults = t->cant_page_faults;
      return 0;
    }
  }
...
#ifdef SC
  printf("Uso LRU - Clock Second Chance\n");
#endif

#ifdef LRU
  printf("Uso LRU - Stack\n");
#endif

  struct thread *curr = thread_current();
  printf("Cantidad de Page faults: %d\n", curr->cant_page_faults);
